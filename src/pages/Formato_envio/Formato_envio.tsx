...
function FormatoEnvio() {
    ...
    const [loadingVendedoras, setLoadingVendedoras] = useState(false);

    // Estados para validación de URLs
    const [isRutaArteUrl, setIsRutaArteUrl] = useState(false);
    const [isCallToActionUrl, setIsCallToActionUrl] = useState(false);
    ...
    const handleChange = async (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        ...
        // Validar si es URL para rutaArte y callToAction
        if (name === 'rutaArte') {
            setIsRutaArteUrl(isValidUrl(value));
        } else if (name === 'callToAction') {
            setIsCallToActionUrl(isValidUrl(value));
        }

        // Si cambia el país, cargar las vendedoras
        if (name === 'pais' && value) {
            setLoadingVendedoras(true);
            try {
                // Mapeo de nombres a códigos de país
                const paisCodigos: { [key: string]: string } = {
                    'México': 'MX',
                    'Colombia': 'CO',
                    'Ecuador': 'EC',
                    'Perú': 'PE'
                ...
                const codigoPais = paisCodigos[value];
                const response = await fetch(`http://localhost:3001/api/vendedoras?pais=${codigoPais}`);
                const data = await response.json();
                setVendedoras(data);

                // Resetear el gerente cuando cambia el país
                setFormData(prev => ({
                    ...prev,
                    [name]: value,
                    gerente: ''
...
Gerente (Mexico) :${formData.gerente || ''}
 </div>
</td>
<td style="width: 50%; vertical-align: top; padding-right: 10px;">
            ...
            <tbody><tr>
                ...
                <td style="width: 75%; vertical-align: top; padding-right: 10px;">
                    ...
                    <div class="tipoBanner" style="width: 33.33%;">
                        <div class="dato-label">Pais:</div>
                        <div class="dato-valor">${formData.pais || ''}</div>
                        </div>
                ...
                <div class="tipoBanner" style="margin-right: 10px;width:33.33%;  ">
                     ...
                     <div class="tipoBanner" style="width: 33.33%;">
                        ...
                        <div class="dato-valor">
                            ${formData.rutaArte && formData.rutaArte.startsWith('http')
                    ? `<a href="${formData.rutaArte}" target="_blank" style="color: #0066cc; text-decoration: none;">🔗 Ver enlace</a>`
                    : formData.rutaArte || ''}
                        </div>
                      </div>
                  </div>
                ...
                <td class="metrica-cell" style="width: 50%; padding: 5px;">
                    <div class="metrica-box">
                        ...
                        <div class="metrica-valor">
                            ${formData.callToAction && formData.callToAction.startsWith('http')
                    ? `<a href="${formData.callToAction}" target="_blank" style="color: #0066cc; text-decoration: none;">🔗 Ver enlace</a>`
                ...
                <Row>
                    <Col md={6}>
                        ...
                        <Row>
                            <Col sm={6}>
                                <Form.Group className="mb-3">
                                    <Form.Label>Gerente {formData.pais && `(${formData.pais})`}</Form.Label>
                                    <Form.Select
                                        name="gerente"
                                        value={formData.gerente}
                                        onChange={handleChange}
                                        disabled={!formData.pais || loadingVendedoras}
                                    >
                                        <option value="">
                                            {loadingVendedoras
                                                ? 'Cargando vendedoras...'
                                        ...
                                        {vendedoras.map((vendedora: any) => (
                                            <option key={vendedora.id} value={vendedora.nombre}>
                                                {vendedora.nombre}
                                            </option>
                                        ))}
                                    </Form.Select>
                            ...
                            <Col sm={6}>
                                <Form.Group className="mb-3">
                                    <div className="d-flex justify-content-between align-items-center mb-2">
                                        <Form.Label className="mb-0">Ruta arte</Form.Label>
                                        {formData.rutaArte && (
                                            <span className={`badge ${isRutaArteUrl ? 'bg-success' : 'bg-secondary'}`}>
                                                {isRutaArteUrl ? '🔗 Link' : '📝 Texto'}
                                            </span>
                                        )}
                                    </div>
                                    <Form.Control
                                        ...
                                        onChange={handleChange}
                                        placeholder="Ingresa la ruta del arte o un link"
                                        required
                                    />
                                </Form.Group>
                            </Col>
                    ...
                    <div className="mb-4">
                        ...
                        <Row className="mb-3">
                            ...
                            <Col md={6}>
                                <Form.Group>
                                    <div className="d-flex justify-content-between align-items-center mb-2">
                                        <Form.Label className="mb-0">Call to action:</Form.Label>
                                        {formData.callToAction && (
                                            <span className={`badge ${isCallToActionUrl ? 'bg-success' : 'bg-secondary'}`}>
                                                {isCallToActionUrl ? '🔗 Link' : '📝 Texto'}
                                            </span>
                                        )}
                                    </div>
                                    <Form.Control
                                        ...
                                        onChange={handleChange}
                                        placeholder="Ingresa el llamado a la acción o un link"
                                    />
                                </Form.Group>
                            </Col>
                        </Row>
